...
# ===========================================================================
# CORRECTION: Gauss clean + harmonic bias
# ===========================================================================
print(f"\n{'='*90}")
...
ex_pre_clean = _get_ex_slice()
q_pre_clean = _measure_gauss_q()
eb_q_pre_clean = _measure_eb_q()
V_pre_clean = corrector.measure_voltages()

print(f"\nBefore correction (step {correction_step}):")
print(f"  V_eff = {V_pre_clean}")
for s in ("left", "right"):
    print(f"  Q_box_{s}_close = {q_pre_clean[f'{s}_close']:.4e}, "
          f"Q_eb_{s} = {eb_q_pre_clean[s]:.4e}")

# --- Step 1: Gauss clean ---
print("\nApplying homogeneous Gauss clean ...", flush=True)
wx_inst.clean_efield_gauss_homogeneous()

ex_post_clean = _get_ex_slice()
q_post_clean = _measure_gauss_q()
eb_q_post_clean = _measure_eb_q()
V_post_clean = corrector.measure_voltages()

print(f"After Gauss clean:")
print(f"  V_eff = {V_post_clean}")

# --- Step 2: Harmonic bias correction ---
print("\nApplying harmonic bias correction ...", flush=True)
V_target = np.array(corrector.v_target)
dV = V_target - corrector.measure_voltages()
print(f"  dV = {dV}")
corrector._apply_bias(dV)

ex_post_bias = _get_ex_slice()
q_post_bias = _measure_gauss_q()
eb_q_post_bias = _measure_eb_q()
V_post_bias = corrector.measure_voltages()

print(f"After harmonic bias:")
print(f"  V_eff = {V_post_bias}")

# ===========================================================================
# PHASE 2: continue with corrected field
# ===========================================================================
print(f"\n{'='*90}")
print(f"PHASE 2: {phase2_steps} steps after correction")
print(f"{'='*90}", flush=True)
...
save_dict = dict(
    ...
    title="Gauss Clean + Harmonic Bias Correction",
    subtitle="Asymmetric beams: passthrough + impact on left electrode, "
             "single correction at step " + str(correction_step),
    ...
    # Two-stage correction snapshots
    correction_step=correction_step,
    ex_pre_clean=ex_pre_clean,
    ex_post_clean=ex_post_clean,
    ex_post_bias=ex_post_bias,
    V_pre_clean=V_pre_clean,
    V_post_clean=V_post_clean,
    V_post_bias=V_post_bias,
)
...
for s in ("left", "right"):
    ...
    save_dict[f"q_{s}_close_pre_clean"] = q_pre_clean[f"{s}_close"]
    save_dict[f"q_{s}_close_post_clean"] = q_post_clean[f"{s}_close"]
    save_dict[f"q_{s}_close_post_bias"] = q_post_bias[f"{s}_close"]
    save_dict[f"eb_q_{s}_pre_clean"] = eb_q_pre_clean[s]
    save_dict[f"eb_q_{s}_post_clean"] = eb_q_post_clean[s]
    save_dict[f"eb_q_{s}_post_bias"] = eb_q_post_bias[s]
