#!/usr/bin/env python3
"""Single-correction test (ECT solver): harmonic bias THEN Gauss clean.
...
followed by one Gauss clean.  Continues 700 steps to check stability.

Usage:
    python inputs_3d_ect_single_bias_clean.py
"""
...
def after_step():
    ...
    if step == correction_step:
        ...
        # --- BIAS THEN CLEAN (reversed order) ---
        ...
        correction_log.append(("bias", step, V_before, V_mid))
        correction_log.append(("clean", step, V_mid, V_after))
        print(f"\n  === CORRECTION at step {step} ({MODE}) ===")
        print(f"  V before:      [{V_before[0]:+.1f}, {V_before[1]:+.1f}]")
        print(f"  V after bias:  [{V_mid[0]:+.1f}, {V_mid[1]:+.1f}]")
        print(f"  V after clean: [{V_after[0]:+.1f}, {V_after[1]:+.1f}]\n",
              flush=True)
        collect(step)
