#!/usr/bin/env python3
"""Single-correction test (ECT solver): Gauss clean ONLY (no harmonic bias).

Two-species plasma pulse through spherical electrodes.
Uses ECT Maxwell solver instead of Yee — EB-aware B-field update with
integral-form Faraday, fewer frozen cells (edge-length-based criterion).
After plasma clears (step 1300), applies one homogeneous Gauss clean.
Continues 700 steps to check stability.

Usage:
    python inputs_3d_ect_single_clean_only.py
"""
...
# ---------------------------------------------------------------------------
# Corrector (needed for voltage measurement even in clean-only mode)
# ---------------------------------------------------------------------------
...
def after_step():
    ...
    if step == correction_step:
        ...
        correction_log.append(("clean", step, V_before, V_after))
