#!/usr/bin/env python3
"""Single-correction test (ECT solver): Gauss clean THEN harmonic bias.

Two-species plasma pulse through spherical electrodes.
Uses ECT Maxwell solver instead of Yee.
After plasma clears (step 1300), applies one Gauss clean followed by
one harmonic bias correction.  Continues 700 steps to check stability.

Usage:
    python inputs_3d_ect_single_clean_bias.py
"""
...
def after_step():
    ...
    if step == correction_step:
        ...
        V_mid = corrector.measure_voltages().copy()
        ...
        correction_log.append(("clean", step, V_before, V_mid))
        correction_log.append(("bias", step, V_mid, V_after))
        print(f"\n  === CORRECTION at step {step} ({MODE}) ===")
        print(f"  V before:      [{V_before[0]:+.1f}, {V_before[1]:+.1f}]")
        print(f"  V after clean: [{V_mid[0]:+.1f}, {V_mid[1]:+.1f}]")
        print(f"  V after bias:  [{V_after[0]:+.1f}, {V_after[1]:+.1f}]\n",
              flush=True)
        collect(step)
