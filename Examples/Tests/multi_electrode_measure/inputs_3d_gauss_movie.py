#!/usr/bin/env python3
"""3D Gauss-box test with E-field movie, density panel, and electrode drift plot.

Neutral beam of equal-mass ± species passes between two spherical electrodes.
The opposite-sign charges split in x under the electrode field, creating a
visible charge separation. After exiting, the field should return to its
initial electrode-only state.

Three nested Gaussian boxes (closed rectangular surfaces) around each sphere
measure the enclosed charge Q at each timestep via the discrete divergence
theorem. The box outlines are drawn on the field/density plots.

No corrector runs — bare FDTD with frozen EB boundary conditions.

Usage:
    python inputs_3d_gauss_movie.py
"""
...
# Neutral beam — equal-mass species with opposite charge
n_beam = 3e14
m_species = m_e
beam_hw = 1.0e-2
z_beam_lo = -half
...
# ---------------------------------------------------------------------------
print("=" * 90)
print("3D GAUSS-LAW MOVIE TEST — neutral equal-mass beam, two spherical electrodes")
print("=" * 90)
...
print(f"Beam: n = {n_beam:.0e}, m = {m_species/m_e:.1f} m_e (both species), "
      f"hw = {beam_hw*1e2:.1f} cm, vz = {v_drift/c_light:.2f}c")
...
# ---------------------------------------------------------------------------
# Neutral beam: equal-mass ± species, same initial positions, same drift
# ---------------------------------------------------------------------------
beam_dist_pos = picmi.AnalyticDistribution(
    density_expression=f"{n_beam}",
    directed_velocity=[0, 0, v_drift],
...
np.savez(
    ...
    z_beam_lo=z_beam_lo, vz_drift=v_drift,
    n_beam=n_beam, beam_hw=beam_hw,
    cells_per_R=cells_per_R,
    title="Gauss-Law Consistency Test",
    subtitle="Two spherical electrodes, neutral equal-mass beam, no corrector",
    beam_desc=f"n = {n_beam:.0e} m^-3, m = m_e (both), "
              f"width = {beam_hw*1e2:.0f} cm, vz = 0.1c",
