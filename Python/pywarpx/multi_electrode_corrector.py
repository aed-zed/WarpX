...
Discrete Shockley--Ramo observer
--------------------------------
The adjoint weighting potentials provide the fast production observation
``Q_g,k = -sum_a rho_a Psi_k[a]`` without a grounded Poisson re-solve.
Set ``qg_mode="reciprocity"`` to use it in the clamp feedback; the grounded
solve remains available as an independent measurement. Setup builds the
adjoint tables automatically when reciprocity or absorption booking needs
them.
...
class MultiElectrodeBiasCorrector:
    ...
    apply_ledger_correction : bool, optional
        ...
        The dynamic 80^3, 2000-step campaign exercised this cumulative ledger
        with every-step correction and showed stable feedback.
    ...
    def __init__(
        ...
        if enable_gauss_clean:
            raise ValueError(
                "enable_gauss_clean=True is not part of the focused harmonic-clamp "
                "implementation; use the bias-only path."
            )
    ...
    def setup_after_init(self):
        ...
        if (self.book_absorption or self.qg_mode == "reciprocity") and (
            self._psi_override is None
        ):
            self.build_adjoint_weighting()
        self._ready = True
        if self.verbose:
    ...
    def measure_voltages(self):
        ...
        *input* (where psi is sampled), never the *arithmetic* this method
        ...
        DYNAMIC VALIDATION. The cumulative ledger was exercised for 2000
        every-step corrections at 80^3 without runaway; combined with the
        reciprocity observer it held the voltage about four times tighter
        than the historical every-ten-step grounded-solve recipe.
