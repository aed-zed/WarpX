...
FullDiagnostics::InitializeFieldFunctorsRZopenPMD (int lev)
{
    ...
    for (int comp=0; comp<m_varname_fields_size; comp++){
        ...
        if ( m_varnames_fields[comp] == "rho" ){
            ...
            // Fallback: the name may be a COMPONENT of a registered VECTOR field
            // (e.g. "Efield_unit_0x" -> vector "Efield_unit_0", Direction{0}).
            // The scalar lookup above does not cover this case; keep it for
            // persistent auxiliary and Python-registered vector fields.
            bool found_in_register = false;
            for (int idir = 0; idir < 3; idir++) {
                std::string base_name = m_varnames_fields[comp];
                if (base_name.size() > 1) {
                    const char last_char = base_name.back();
            ...
            if (!found_in_register) {
                WARPX_ABORT_WITH_MESSAGE(
                    ...
                    "If using auxiliary WarpX-internal fields, ensure the corresponding solvers are active "
