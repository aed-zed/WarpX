        ...
        for (auto const & [field_name, maybe_dir] : m_custom_fields_to_restore[lev]) {
            ...
            if (maybe_dir.has_value()) {
                ...
                const auto prefix = amrex::MultiFabFileFullPrefix(
                    lev, restart_chkfile, "Level_", checkpoint_name);
                VisMF::Read(*m_fields.get(field_name, dir, lev), prefix);
            } else {
                const auto prefix = amrex::MultiFabFileFullPrefix(
                    lev, restart_chkfile, "Level_", field_name);
                VisMF::Read(*m_fields.get(field_name, lev), prefix);
            }
        }
    }
}
